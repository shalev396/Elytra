...
def test_login_loads(page: Page, app_url: str):
    ...
    expect(page.get_by_label("Password")).to_be_visible(timeout=15000)
    expect(page.get_by_role("button", name="Login")).to_be_visible(timeout=15000)
...
def test_login_form_elements(page: Page, app_url: str):
    ...
    page.goto(f"{app_url}/auth/login", wait_until="domcontentloaded")
    page.wait_for_load_state("networkidle")
    expect(page.get_by_text("Welcome back")).to_be_visible(timeout=15000)
