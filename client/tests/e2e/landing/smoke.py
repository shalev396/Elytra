...
def test_landing_loads(page: Page, app_url: str):
    ...
    expect(page.get_by_role("heading").first).to_be_visible(timeout=15000)
    expect(page.get_by_role("navigation").first).to_be_visible(timeout=15000)
...
def test_app_shell_renders(page: Page, app_url: str):
    ...
    page.wait_for_load_state("networkidle")
    expect(page.get_by_role("navigation").first).to_be_visible(timeout=15000)
    expect(page.get_by_role("contentinfo")).to_be_visible(timeout=15000)
    expect(page.get_by_role("main").first).to_be_visible(timeout=15000)
...
def test_hero_section(page: Page, app_url: str):
    ...
    page.goto(app_url, wait_until="domcontentloaded")
    page.wait_for_load_state("networkidle")
    expect(page.get_by_text("Open Source Full-Stack Template")).to_be_visible(timeout=15000)
    expect(page.get_by_text("Build Serverless Apps")).to_be_visible(timeout=15000)
...
def test_navbar_and_footer(page: Page, app_url: str):
    ...
    expect(page.get_by_role("link", name="Go to home").first).to_be_visible(timeout=15000)
    expect(page.get_by_role("link", name="Get Started")).to_be_visible(timeout=15000)
    footer = page.get_by_role("contentinfo")
    footer.scroll_into_view_if_needed(timeout=10000)
    expect(footer.get_by_role("link", name="Terms of Service")).to_be_visible(timeout=15000)
    expect(footer.get_by_role("link", name="Privacy Policy")).to_be_visible(timeout=15000)
